...
const CLASS_DRAG_ACTIVE = "vjs-drag-active"
...
export class DragManager {
    ...
    constructor(surface:Surface) {
        ...
        const dragstartHandler = (ev:any) => {
            ...
            this.currentNode = surface.getModelObjectFromElement(ev.target, true, false)?.obj as Node
        ...
        surface.bind(EVENT_NODE_RENDERED, (p:NodeRenderedParams<BrowserElement>) => {
            // initialise the element for native drag
            p.el.addEventListener(EVENT_DRAG_START, dragstartHandler)
            p.el.addEventListener(EVENT_DRAG_END, (e:Event) => {
                (e.target as any).classList.remove(CLASS_DRAG_ACTIVE)
            })
        })
        ...
        //
        const initGroupOver = (el:BrowserElement) => {
            return (event:any) => {
        ...
        const initGroupDrop = (el:BrowserElement, group:Group) => {
            return (evt:MouseEvent) => {
                ...
                if(group.id === this.currentColumn.id) {
                    ...
                    surface.model.addToGroup(this.currentNode, group)
                }
            }
        }
        ...
        surface.bind(EVENT_GROUP_RENDERED, (p:GroupRenderedParams<BrowserElement>) => {
            ...
            const enterHandler = initGroupOver(p.el);
            ...
            this.eventManager.on(p.el, EVENT_DRAG_LEAVE, () => {
        ...
        const _reorderGroup = (group:Group) => {
            ...
            if (el != null) {
                ...
                const contentArea = surface.$getGroupContentAreaElement(el)
        ...
        surface.bind(EVENT_NODE_REMOVED, (p:NodeRemovedParams) => {
            if (p.parentGroup) {
                _reorderGroup(p.parentGroup)
            }
        })

        surface.model.bind(EVENT_GROUP_MEMBER_ADDED, (p:GroupMemberAddedParams) => {
